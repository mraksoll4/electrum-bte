...
;Variables
  ...
  !define PRODUCT_NAME "Electrum Bitweb"
  !define PRODUCT_WEB_SITE "https://github.com/bitweb-project/electrum-bte"
  !define PRODUCT_PUBLISHER "Bitweb Electrum"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-bte-setup.exe"
  ...
  VIAddVersionKey LegalCopyright "2021-2023 ${PRODUCT_PUBLISHER}"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bte-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bte-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-bte-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-bte-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-bte-${PRODUCT_VERSION}.exe" 0


  ;Links bitweb: and lightning: URIs to Electrum
  WriteRegStr HKCU "Software\Classes\bitweb" "" "URL:bitweb Protocol"
  WriteRegStr HKCU "Software\Classes\bitweb" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitweb" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitweb\shell\open\command" "" "$\"$INSTDIR\electrum-bte-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-bte-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\bitweb"
